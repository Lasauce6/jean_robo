...
def duration_to_str(duration):
    ...
    duration = []
    if days > 0: duration.append(f'{days} jours')
    if hours > 0: duration.append(f'{hours} heures')
    if minutes > 0: duration.append(f'{minutes} minutes')
    if seconds > 0 or len(duration) == 0: duration.append(f'{seconds} secondes')
...
class SongInfo:
    ...
    def from_file(cls, file, requester, channel):
        ...
        if not path.exists():
            raise MusicError(f"Le fichier {file} n'existe pas !")
    ...
    async def from_ytdl(cls, request, requester, channel, loop=None):
        ...
        if sparse_info is None:
            raise MusicError(f"Keske tu raconte ? C'est pas francais : {request}")
        ...
        if "entries" not in processed_info:
            ...
            while info is None:
                try:
                    ...
                    raise MusicError(f'Je comprend rien de ton url : {info_to_process["url"]}')
...
class GuildMusicState:
    ...
    async def play_next_song(self, song=None, error=None):
        ...
        if error:
            await self.current_song.channel.send(f'Oupsi une petite erreur à eu lieu lors de mon interprétation de {self.current_song}: {error}')
...
class Music(commands.Cog):
    ...
    def cog_check(self, ctx):
        ...
        if not ctx.guild:
            raise commands.NoPrivateMessage("Cette commande n'est pas une commande de message privé !")
        return True
    ...
    async def nowplaying(self, ctx):
        ...
        if ctx.music_state.is_playing():
            ...
            await ctx.send(f"Je vous interprète {song}.\nJe beugle à {song.volume * 100}% dans {ctx.voice_client.channel.mention}")
        else:
            await ctx.send("Je vous intèrprète de la musique notemment")
    ...
    async def join(self, ctx, *, channel: discord.VoiceChannel = None):
        ...
        if channel is None and not ctx.author.voice:
            raise MusicError("T'es même pas dans un channel je peux pas te rejoindre")
    ...
    async def play(self, ctx, *, request: str):
        ...
        try:
            ...
            raise MusicError('Y a trop de musiques ! Je ne puis plus !')
    ...
    async def skip(self, ctx):
        ...
        """
        if not ctx.music_state.is_playing():
            raise MusicError('Je joue pas ouesh tu veux quoi !')

        if ctx.author.id in ctx.music_state.skips:
            raise MusicError(f'{ctx.author.mention} Tu a déja voté pour skip fait pas le fou avec moi !')
